### daily mm catchment-wide

for (i in 1:9){
  
  ir <- sum(WSI[[i]]$irr_median)
  ir <- ir/365.25
  
  print(paste(names(WSI[i]),ir, sep = ": ")) 
  
}



## crop reference ET0 for 10.March - October 
tf_vec <- year(ET0_CTM$date)>=1990&year(ET0_CTM$date)<=2019
ET0_BER[tf_vec,]

ir <- monthly_sums(ET0_BER[tf_vec,])

for(i in 1:3){
  print(
    paste(names(ir)[i],
        sum(c(ir[[i]]$median[4:10],(ir$RCP26$median[3]/2))),
        sep = ": "
        )
  )
...
yearly_sums(ET0_CTM,periods = periods)
...
k = 14 # Define the window size for the rolling mean
...
irrigation_need_CTM_TEST <- data.frame(date = ETc_CTM$date)
for(i in 2:ncol(ETc_CTM)){
  ...
  irrigation_need_CTM_TEST[[irg_name]] <- irg_irr_need
}
...
irrigation_need_CTM_TEST <- data.frame(date=date_df$date,
                                  lapply(irrigation_need_CTM_TEST[2:dim(irrigation_need_CTM_TEST)[2]], function(x) x * date_df$irrigated_area))

# divide by Catchment size to get demand in l/m2 (== mm catchment-wide) 
size_catchment_m2 <-  116.1 * 10^6
irrigation_need_CTM_TEST <- data.frame(date=date_df$date,
                                  lapply(irrigation_need_CTM_TEST[2:dim(irrigation_need_CTM_TEST)[2]], function(x) x / size_catchment_m2))



yearly_sums(irrigation_need_CTM_TEST, periods = periods)


periods <-data.frame(start=c(1990,2020,2045),end=c(2019,2049,2074))

yearly_sums(streamflow_belp_mod_mm, periods = periods)
